...
#        cv2.destroyAllWindows()
    ...
    def add_person(self):
        if len(os.listdir(AUDIODIR)) >= 200:#100
            ...
            return
        try:
            play_machine_voice.play_machine_audio("please_tell_the_name.mp3")
            name = get_name()
...
#                print(person_id)
                ...
                if status:
                    ...
                    delete_face(name)
                    subprocess.run(["rm", "-r", audio_path])
                    subprocess.run(["rm", "-r", f"{EMBEDDINGS_DIR}/{name}.npy"])
                    subprocess.run(["rm", "-r", img_dir])
...
#                play_machine_voice.play_machine_audio("check_your_connection_and_proceed.mp3")
            ...
            img_path = '/home/rock/Desktop/Hearsight/English/set_face_who/2.jpg'
